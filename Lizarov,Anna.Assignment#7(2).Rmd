---
title: "Lizarov, Anna. Assignment#7(2). Rmd"
author: "Anna Lizarov"
...
```{r}
#Answer: Model 2 is better since its area under the ROC (AUC) is greater than that of model 1. In particular, AUC for model 2 is 0.977 whereas the AUC for model 1 is 0.855. This suggests that model 2 is better at predicting that the system will allow the student to go to the next level. 
```
...
```{r}
# Answer: For both models, different threshold values produced the same results. However, Model 2 is better than Model 1 when it comes to the diagnostic metrics. This indicates that not only model 2 is better at predicting that the system will allow the student to go to the next level, it produces more accurate predictions. 
```
